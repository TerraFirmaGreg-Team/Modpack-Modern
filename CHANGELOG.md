...
## [0.3.0] - До нового года, емаеее
### Изменения
- Отключены все руды, которые не используются в игре, повысит производительность и уменьшит использование ОЗУ.
- Изменены ключи локализации руд.
- Изменен тэг для руд по которому добавляется для них осыпание, триггер осыпания и другое.
- В 3-ий раз меняю, теперь снова используются ванильные слитки золота и меди в рецептах (это последний раз).
...
- Теперь латекс можно переделать в резину быстрее, используя сепаратор.
- Отключены все бронзовые машины, теперь пар начинается со стали (В противном случае пропускается большая часть контента тфк, тк бронза открывает сразу путь к машинам, которые могут тебе давать и чистую руду и ковать слитки и многое другое), скорее всего в будущем в место бронзовой эры, будет механическая эра из Create, которая позволит облегчить страдания игроков, но это уже в след. патче.
- Переписаны все рецепты ваниллы, надеюсь что все + скрыты все предметы, которые вы не должны или не сможете использовать из ваниллы, чтобы не мозолили глаза.
- Скрыты дополнительно, некоторые механические предметы и блоки из TFC, о которых я забыл в прошлом патче.
- Переписаны наконец-то все рецепты FirmaLife, теперь вы сможете сделать теплицу, ура.
- Ну и еще что-то потом сюда добавлю, ну а пока все, подписывайтесь на канал, ставьте лайки.
### Новые моды
- FirmaCivilization
- Steam_Rails
- ad_astra
