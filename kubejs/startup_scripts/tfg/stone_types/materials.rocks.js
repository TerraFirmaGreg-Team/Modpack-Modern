...
const registerTFGRockMaterials = (event) => {
    ...
    const registerStoneMaterial = (rockType, color, components, fluid, temp) => {
        let builder = event.create(`tfg:${rockType}`)
            .color(color)
            .dust()
        ...
        if (fluid === true) {
			let fluidBuilder = GTFluidBuilder();
			if (temp !== undefined) {
				fluidBuilder.temperature(temp);
			}
			builder.liquid(fluidBuilder);
		}
    };
    ...
    registerStoneMaterial('igneous_felsic', 0x97B2BF, ['granite_red'], true, 1346);
}