...
const $FastNoiseLite = Java.loadClass("net.dries007.tfc.world.noise.FastNoiseLite")
...
TFCEvents.createChunkDataProvider('mars', event => {
	...
	//let rockNoise = new $FastNoiseLite(event.worldSeed + 8008135)
	//rockNoise.SetNoiseType($FastNoiseLite.NoiseType.Cellular)
	//rockNoise.SetCellularReturnType($FastNoiseLite.CellularReturnType.CellValue)
	...
	event.rocks((x, y, z, surfaceY, cache, rockLayers) => {
		...
		//return rockLayers.sampleAtLayer(rockNoise.GetNoise(x, z) * 0x7fffffff, (surfaceY - y) / ROCK_LAYER_HEIGHT);
	});
})
